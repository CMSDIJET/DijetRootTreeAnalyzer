...
parser.add_argument("--Suu", type=float, default=8400.0, help="Suu mass")
parser.add_argument("--Chi", type=float, default=2100.0, help="Chi mass")
...
#pt = ROOT.TPaveText(0.3, 0.48, 0.6, 0.7, 'NDCNB')
pt = ROOT.TPaveText(0.13, 0.67, 0.41, 0.91, 'NDCNB')
pt.AddText(textfit)
...
#plot.DrawCMSLogo(pads[0], args.logo, args.logo_sub, 10, 0.045, 0.035, 1.2, cmsTextSize=1.0)

legend_l = 0.69
if len(other_scans) > 0:
    legend_l = legend_l - len(other_scans) * 0.04
    #legend = ROOT.TLegend(0.15, legend_l, 0.45, 0.78, "", "NBNDC")
    legend = ROOT.TLegend(0.55, 0.73, 0.95, 0.83, "", "NBNDC")
if len(other_scans) >= 3:
...
lumi_leg = ROOT.TPaveText(0.807,0.955,0.907,0.985,"NDC")
lumi_leg.AddText("138 fb^{-1} (13 TeV)")
lumi_leg.SetBorderSize(0)
lumi_leg.SetFillColor(0)
lumi_leg.SetTextSize(0.04)
lumi_leg.SetTextFont(42)
lumi_leg.Draw()

cms_leg = ROOT.TPaveText(0.11,0.955,0.31,0.985,"NDC")
cms_leg.AddText("#bf{CMS} #scale[0.7]{#it{Preliminary}}")
cms_leg.SetBorderSize(0)
cms_leg.SetFillColor(0)
cms_leg.SetTextSize(0.045)
cms_leg.SetTextFont(42)
cms_leg.Draw()
