...
do
    ...
    do
   	...
   	do
	    ...
	    combine -M Significance output/Wide_Resonances_Run2_Final/datacards_alpha0p"$alphap"_refxsec0p01pb/card_combined_W-"$width"_"$Suu"_"$Chi".txt --rMin 0 --rMax "$rmax" --saveWorkspace -n _alpha0p"$alphap"_W-"$width"_"$Suu"_"$Chi"

	    mv higgsCombine_alpha0p"$alphap"_W-"$width"_"$Suu"_"$Chi".Significance.mH120.root output/Wide_Resonances_Run2_Final/combine_rootfiles_alpha0p"$alphap"/significances/
       done
   done
done
