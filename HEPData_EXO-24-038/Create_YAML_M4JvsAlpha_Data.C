...
void Create_YAML_M4JvsAlpha_Data()
{

    TFile *f = new TFile("inputs/HISTOS_2016_2017_2018.root", "READ");
    TH2D* h = (TH2D*)f->Get("h_alpha_vs_M2J");
    
    char yamlFileName[1024];
    sprintf(yamlFileName, "outputs/Figure_002-b_M4JvsAlpha_Data.yaml");
    ...
    const double yMin = 0.10, yMax = 0.50;
    ...
    for (int ix = 1; ix <= h->GetNbinsX(); ++ix) {
        ...
        for (int iy = 1; iy <= h->GetNbinsY(); ++iy) {
            ...
            if (ycenter <= yMin || ycenter >= yMax) continue;
    ...
    for (const auto& bin : validBins) {
        ...
        fout << "      - value: " << std::fixed << std::setprecision(1) << val << "\n";
    }
    ...
    // Y axis
    fout << "  - header: {name: $\\alpha$ = Average dijet mass / Four-jet mass}\n";
    fout << "    values:\n";
    for (const auto& bin : validBins) {
        int iy = bin.second;
        double low = h->GetYaxis()->GetBinLowEdge(iy);
        double high = h->GetYaxis()->GetBinLowEdge(iy+1);
        fout << std::fixed << std::setprecision(2) << "      - {low: " << low << ", high: " << high << "}\n";
    }
