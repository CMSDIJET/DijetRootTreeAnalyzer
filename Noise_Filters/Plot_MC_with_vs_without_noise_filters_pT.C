...
void Plot_MC_with_vs_without_noise_filters_pT(){

char xaxistitle[1024];
char run[1024];

sprintf(xaxistitle,"p_{T} of the 2 wide jets [GeV]");
double xmin = 1;
double xmax = 5000;
...
sprintf(fname1,"Dijet_Analysis/Histos_QCDFlatMC_AllSelCrit_AK4PUPPI_withoutNoiseFilters.root");
sprintf(fname2,"Dijet_Analysis/Histos_QCDFlatMC_AllSelCrit_AK4PUPPI_withBadChargedCandidate.root");

TFile *f1 = new TFile(fname1,"READ");
TFile *f2 = new TFile(fname2,"READ");


TH1D *h1 = (TH1D*)f1->Get("h_pT_mc");
TH1D *h2 = (TH1D*)f2->Get("h_pT_mc");
...
sprintf(integNoFilt,"Integral NoFilter = %.5f",h1->Integral());
sprintf(integWithFilt,"Integral WithFilter = %.5f",h2->Integral());

//TLegend *leg = new TLegend(0.45,0.5,0.75,0.9,""); //top right
//TLegend *leg = new TLegend(0.5,0.05,0.7,0.45,""); //bottom right
TLegend *leg = new TLegend(0.2,0.05,0.5,0.5,""); //bottom left
//TLegend *leg = new TLegend(0.36,0.05,0.56,0.45,""); //bottom middle
leg->SetBorderSize(0);
leg->SetTextSize(0.05);
leg->AddEntry((TObject*)0,"AK4 PUPPI","");
//leg->AddEntry((TObject*)0,"#splitline{All selection criteria}{+ MET / H_{T} < 0.2}","");
...
sprintf(cname_1,"Dijet_Analysis_new/AllCuts_QCDMC_with_VS_without_BadChargedCandidate_pTwj.png");
sprintf(cname_2,"Dijet_Analysis_new/AllCuts_QCDMC_with_VS_without_BadChargedCandidate_pTwj.pdf");
