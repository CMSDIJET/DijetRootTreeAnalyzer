...
def getThyXsecDict(width_name, alphap):    
    ...
    xsecFiles = ['files/DiquarkSuu_W-%s_0p%.0f.txt'%(width_name, alphap), 'files/DiquarkSddCorrWithPartonLevelAcc_W-%s_0p%.0f.txt'%(width_name, alphap)]
    ...
    #for (mass,thyXsec) in thyXsecDict['DiquarkSuu'].items():
    #    thyXsecDict['DiquarkSuu'][mass] = 1.0 * thyXsec
    return thyXsecDict
...
def getHybridCLsArrays(directory, model, Box, bayes,alphap,width_name):
    if bayes:
        tfile = rt.TFile.Open("%s/xsecUL_MarkovChainMC_%s_%s.root"%(directory,model,Box))
    else:
        tfile = rt.TFile.Open("%s/xsecUL_HybridNew_%s_alpha0p%.0f_W-%s_%s.root"%(directory,model,alphap,width_name,Box))
    ...
    while True:
        ...
        # Dictionary to capture local variables
...
def getSignificanceArrays(directory, model, Box,alphap, width_name):
    tfile = rt.TFile.Open("%s/xsecUL_HybridNew_%s_alpha0p%.0f_%s.root"%(directory,model,alphap,width_name,Box))
    xsecTree = tfile.Get("xsecTree")
...
def setstyle():
    ...
    rt.gStyle.SetTitleSize(0.055,"xyz") # set the 3 axes title size
    rt.gStyle.SetTitleSize(0.06," ")   # set the pad title size
    rt.gStyle.SetTitleSize(0.06,"y")   # set the pad title size
    rt.gStyle.SetTitleOffset(1.3,"y")   # set the pad title size
    rt.gStyle.SetTitleOffset(1.05,"x")   # set the pad title size
...
if __name__ == '__main__':
    ...
    if options.model=='gg':
        ...
        print (options.box)       
        if 'PF' in options.box:
            thyModelsToDraw = ['DiquarkSuu', 'DiquarkSddCorrWithPartonLevelAcc']           
    ...
    lineStyle = {'RSGravitonGG':4,
                 ...
                 "String":7,     
                 "DiquarkSuu":5,
                 "DiquarkSddCorrWithPartonLevelAcc":5,
                 "DM1GeV": 8,
                 "DMbb1GeV": 8,
                 'None':1               
                 }
    ...
    lineColor = {'RSGravitonGG':rt.kGray+1,
                 ...
                 "DMbb1GeV":rt.kViolet,
                 "String":rt.kAzure-3,
                 "DiquarkSuu":rt.kRed+2,
                 "DiquarkSddCorrWithPartonLevelAcc":rt.kGreen+2,
                 'None':1,
                 'gg':rt.kGreen+1,
                 'qq':rt.kRed,
                 'qg':rt.kBlue,
    ...
    legendLabel = {'RSGravitonGG':'RS graviton (gg#rightarrowG#rightarrowgg)',
                   ...
                   "DMbb1GeV": "DM mediator to bb",
                    "String": "String",
                    "DiquarkSuu":"uu #rightarrow S_{uu} #rightarrow #chi#chi #rightarrow (ug)(ug)",
                    "DiquarkSddCorrWithPartonLevelAcc":"dd #rightarrow S_{dd} #rightarrow #omega#omega #rightarrow (dg)(dg)",
                   'gg':'gluon-gluon',
                   'qq':'Suu->#chi #chi',
                   'qg':'quark-gluon',
    ...
    for Box in Boxes:
        for model in models:
            if not model=='rsg':
                ...
                if options.doSignificance:
                    gluinoMassArray[(Box,model)], gluinoMassArray_er[(Box,model)], observedLimit[(Box,model)], observedLimit_er[(Box,model)], expectedLimit[(Box,model)], expectedLimit_minus1sigma[(Box,model)], expectedLimit_plus1sigma[(Box,model)], expectedLimit_minus2sigma[(Box,model)], expectedLimit_plus2sigma[(Box,model)] = getSignificanceArrays(directory, model, Box,alphap,width_name)
                else:        
                    gluinoMassArray[(Box,model)], gluinoMassArray_er[(Box,model)], observedLimit[(Box,model)], observedLimit_er[(Box,model)], expectedLimit[(Box,model)], expectedLimit_minus1sigma[(Box,model)], expectedLimit_plus1sigma[(Box,model)], expectedLimit_minus2sigma[(Box,model)], expectedLimit_plus2sigma[(Box,model)] = getHybridCLsArrays(directory, model, Box, options.bayes,alphap,width_name)
            else:
...
#                  expectedLimit_minus2sigma_new[i] = expectedLimit_minus2sigma[(Box,model)][i]
    ...
    if options.doSignificance:
        ...
        leg_SigProc = rt.TLegend(0.66,0.63,0.93,0.63)
        leg_alpha = rt.TLegend(0.686,0.57,0.965,0.57)
        leg_width = rt.TLegend(0.702,0.51,0.97,0.51) #0.69-0.97, 0.72-0.97
        #leg_arxiv = rt.TLegend(0.27,0.18,0.57,0.28)
        leg_arxiv = rt.TLegend(0.2,0.15,0.5,0.25)
    ...
    leg_width.SetFillColorAlpha(0,0)
    leg_width.SetLineColor(0)
    leg_arxiv.SetTextFont(42)
    leg_arxiv.SetFillColorAlpha(0,0)
    leg_arxiv.SetLineColor(0)
	...
	#leg_arxiv.SetHeader("#splitline{y_{uu}=0.4, y_{#chi}=0.6}{arXiv:1810.09429}")
        if(width_name=='0p0043'): 
            leg_arxiv.SetHeader("y_{uu} = y_{dd} = 0.4, y_{#chi} = y_{#omega} = 0.6")
        elif(width_name=='0p015'):
            leg_arxiv.SetHeader("y_{uu} = y_{dd} = 0.747, y_{#chi} = y_{#omega} = 1.120") 
        elif(width_name=='0p05'):
            leg_arxiv.SetHeader("y_{uu} = y_{dd} = 1.363, y_{#chi} = y_{#omega} = 2.045") 
        elif(width_name=='0p1'):
            leg_arxiv.SetHeader("y_{uu} = y_{dd} = 1.928, y_{#chi} = y_{#omega} = 2.892")           
    ...
    if len(thyModelsToDraw)>0 and not options.doSignificance:        
        if options.model =="gg_qg_qq" and options.box=="CaloDijet2016_PFDijet2016":
            ...
            #legThyModel = rt.TLegend(0.53,0.55,0.91,0.70)
            #legThyModel = rt.TLegend(0.2,0.18,0.55,0.4)
            legThyModel = rt.TLegend(0.19,0.23,0.44,0.315)
    ...
    if options.doSignificance:
        ...
        if options.bayes:
            ...
            if options.noSys:
                ...
                for limitType, graphs in graphDict.items(): 
                    for (Box,model), graph in graphs.items():
                        graph.SetName('%s_%s_%s'%(limitType,model,Box.lower()))
                        graph.Write()
            else:
                #c.SaveAs("plots/AsymptPlusHNLimitVsMass_alpha0p"+str(int(alphap))+"_W-"+width_name+".png")
                #c.SaveAs("plots_PAS/AsymptPlusHNLimitVsMass_alpha0p"+str(int(alphap))+"_W-"+width_name+".pdf")
                c.SaveAs("plots/Figure_008-c.pdf")
                #c.SaveAs("plots/AsymptPlusHNLimitVsMass_alpha0p"+str(int(alphap))+"_W-"+width_name+".C")
                #c.SaveAs("Figure_011.pdf")
                #outFile = rt.TFile.Open(options.outDir+"/AsymptPlusHNLimitVsMass_alpha0p"+str(int(alphap))+"_W-"+width_name+"_PAS.root","recreate")
                outFile = rt.TFile.Open(options.outDir+"/AsymptPlusHNLimitVsMass_alpha0p"+str(int(alphap))+"_W-"+width_name+"_PAPER.root","recreate")
                outFile.cd()
                c.Write()
                for thyModel in thyModelsToDraw: 
