...
void Plot_PValues_vsWidth_alpha0p29(){

char fname_W0p015[1024], fname_W0p05[1024], fname_W0p1[1024], fname_W0p0043[1024];
sprintf(fname_W0p0043, "files/pvalue_qq_pfdijetrun2_alpha0p29_W-0p0043_AsymptPlusHN.root");
sprintf(fname_W0p015, "files/pvalue_qq_pfdijetrun2_alpha0p29_W-0p015_AsymptPlusHN.root");
sprintf(fname_W0p05, "files/pvalue_qq_pfdijetrun2_alpha0p29_W-0p05_AsymptPlusHN.root");
sprintf(fname_W0p1, "files/pvalue_qq_pfdijetrun2_alpha0p29_W-0p1_AsymptPlusHN.root");
...
char alpha_leg[1024];
sprintf(alpha_leg, "#bf{M_{X} / M_{Y} = 0.29}");

TLegend *leg = new TLegend(0.48,0.21,0.78,0.41);
leg->SetFillColor(0);
leg->SetBorderSize(0);
...
TLine *l0sigma = new TLine(2000, 0.5, 4300, 0.5); 
TLine *l1sigma = new TLine(2000, 0.15865525, 4300, 0.15865525);
TLine *l2sigma = new TLine(2000, 0.022750132, 4300, 0.022750132);
TLine *l3sigma = new TLine(2000, 0.0013498980, 4300, 0.0013498980);
TLine *l4sigma = new TLine(2000, 3.1671242e-05, 4300, 3.1671242e-05);   
...
TLine *l0sigma_v2 = new TLine(5000, 0.5, 10000, 0.5); 
TLine *l1sigma_v2 = new TLine(5000, 0.15865525, 10000, 0.15865525);
TLine *l2sigma_v2 = new TLine(5000, 0.022750132, 10000, 0.022750132);
TLine *l3sigma_v2 = new TLine(5000, 0.0013498980, 10000, 0.0013498980);
TLine *l4sigma_v2 = new TLine(5000, 3.1671242e-05, 10000, 3.1671242e-05);   
...
lsigma_leg->DrawLatex(4650, 0.15865525, "1 s.d."); 
lsigma_leg->DrawLatex(4650, 0.022750132, "2 s.d.");
lsigma_leg->DrawLatex(4650, 0.0013498980, "3 s.d.");
lsigma_leg->DrawLatex(4650, 3.1671242e-05, "4 s.d.");


char cname[1024];
sprintf(cname,"plots/Figure_011-b.pdf");
