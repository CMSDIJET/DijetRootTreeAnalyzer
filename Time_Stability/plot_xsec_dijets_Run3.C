...
void plot_xsec_dijets_Run3()
{ 
   char name[1000],dirname[1000];

   TFile *inf; 
   inf = TFile::Open("output_dijets/Run2022BCDEF_29p6fb-1_dijets_stability_cert_new.root");
   ...
   hbig->GetXaxis()->SetRangeUser(-50.,350.);
   hbig->SetMarkerStyle(8);
   hbig->SetMarkerColor(kBlack);
   hbig->SetLineColor(kBlack);
   hbig->Draw("pe1");

   TLegend *leg1 = new TLegend(0.16,0.6,0.16,0.87,"");
   leg1->SetTextSize(0.04);
   leg1->SetBorderSize(0);
   leg1->AddEntry((TObject*)0,"Dijet analysis","");
   leg1->AddEntry((TObject*)0,"Wide jets","");
   leg1->AddEntry((TObject*)0,"|#eta| < 2.5","");
   leg1->AddEntry((TObject*)0,"leading p_{T} > 60 GeV","");
   leg1->AddEntry((TObject*)0,"sub-leading p_{T} > 30 GeV","");
   leg1->Draw();

   TLegend *leg2 = new TLegend(0.6,0.7,0.6,0.8,"");
   leg2->SetTextSize(0.04);
   leg2->SetBorderSize(0);
   leg2->AddEntry((TObject*)0,"M_{jj} > 1607 GeV","");
   ...
   for(int i=1; i<=hbig->GetNbinsX(); i++)
   {
	cout << "Bin center " << i << " = " << hbig->GetBinCenter(i) << endl;
   }
   ...
   c->SaveAs("output_dijets/dijets_xsec_vs_run.png");
   c->SaveAs("output_dijets/dijets_xsec_vs_run.pdf");
