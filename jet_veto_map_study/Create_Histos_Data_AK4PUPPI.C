...
h_etaphi_data = new TH2D("h_etaphi_data","",nEtaBins,EtaBins,72,-3.1415927,3.1415927);

double run, lumi, event, nVtx, PassJSON;
...
//for (int i=0; i<100000; i++){
	...
	if(FourjetMass>1607 && Masymmetry<0.1 && DeltaEtaJJ<1.1 && DeltaR_First<2.0 && DeltaR_Second<2.0 && alpha>0.1)
	{
		...
		h_etaphi_data->Fill(etaj1,phij1);
		h_etaphi_data->Fill(etaj2,phij2);
		h_etaphi_data->Fill(etaj3,phij3);
		h_etaphi_data->Fill(etaj4,phij4);

	}

}// end of event loop

//TFile *fout = new TFile("histos/Histos_AK4PUPPI_2022EFG_NoVetoMap.root","RECREATE");
//TFile *fout = new TFile("histos/Histos_AK4PUPPI_2022EFG_VetoMapEE.root","RECREATE");
TFile *fout = new TFile("histos/Histos_AK4PUPPI_2022EFG_VetoMapAll.root","RECREATE");
fout->cd();
