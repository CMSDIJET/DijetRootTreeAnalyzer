...
from WriteDataCard_4jets import initializeWorkspace,convertToTh1xHist,convertToMjjHist,applyTurnonFunc,applyTurnonGraph
import os
import sys
import random
import math
...
def calculateChi2AndFillResiduals(data_obs_TGraph_,background_hist_,hist_fit_residual_vsMass_,workspace_,prinToScreen_=0,effFit_=False):
    ...
    if workspace_.var('meff_%s'%box).getVal()>0 and workspace_.var('seff_%s'%box).getVal()>0 :
        nParFit = 6
    if workspace_.var('p54_%s'%box) != None or workspace_.var('p4_%s'%box) != None or workspace_.var('pa4_%s'%box) != None :
       # if workspace_.var('pm4_%s'%box) != None and workspace_.var('pm4_%s'%box).getVal()==0:
           # nParFit = 4
        if workspace_.var('p4_%s'%box) != None and workspace_.var('p4_%s'%box).getVal()==0 and workspace_.var('p3_%s'%box) != None and workspace_.var('p3_%s'%box).getVal()==0 and workspace_.var('p2_%s'%box) != None and workspace_.var('p2_%s'%box).getVal()==0:
            nParFit = 2
        elif workspace_.var('p4_%s'%box) != None and workspace_.var('p4_%s'%box).getVal()==0 and workspace_.var('p3_%s'%box) != None and workspace_.var('p3_%s'%box).getVal()==0:
            nParFit = 3
        elif workspace_.var('p4_%s'%box) != None and workspace_.var('p4_%s'%box).getVal()==0:
            nParFit = 4
        else:
            nParFit = 5
    ...
    for bin in range (0,N_massBins_):
        ...
        binWidth_current = xbinHigh - xbinLow
	#print ('Data val: ', value_data*137000.*binWidth_current)
	#print ('Err Up: ', err_high_data*137000.*binWidth_current)
	#print ('Err Low: ', err_low_data*137000.*binWidth_current)
...
if __name__ == '__main__':
    ...
    if options.triggerDataFile is not None:        
        ...
        l.DrawLatex(0.73,0.96,"%.1f fb^{-1} (%.1f TeV)"%(lumi/1000.,w.var('sqrts').getVal()/1000.))     
        # PAS        
        #l.SetTextFont(62)
        #l.SetTextSize(0.055)   
    ...
    if 'PF' in box:
        myRebinnedDensityTH1.SetMaximum(2)#20
    ...
    l.SetNDC()
    #l.DrawLatex(0.7,0.96,"%i pb^{-1} (%i TeV)"%(lumi,w.var('sqrts').getVal()/1000.))
    l.DrawLatex(0.72,0.96,"%.1f fb^{-1} (%.1f TeV)"%(lumi/1000.,w.var('sqrts').getVal()/1000.))
    # PAS
    #l.SetTextFont(62)
    #l.SetTextSize(0.055)   
    ...
    if 'Calo' in box:
        ...
        pave_sel.AddText("AK4 PUPPI Jets")
        #pave_sel.AddText("Wide PF-jets")        
        #pave_sel.AddText("%.1f < m_{jj} < %.1f TeV"%(w.var('mjj').getMin('Low')/1000.,w.var('mjj').getMax('High')/1000.))
        pave_sel.AddText("m_{4j} > %.2f TeV"%(w.var('mjj').getMin('Low')/1000.))
...
# w.var('p3_%s'%box).getVal(), (w.var('p3_%s'%box).getErrorHi() - w.var('p3_%s'%box).getErrorLo())/2.0,
    ...
    if 'PF' in box:
        ...
        yLab.DrawLatex(xM, 0.00000000001, "10^{#minus8}")
        yLab.DrawLatex(xM, 0.000000000001, "10^{#minus9}")
    ...
    if 'PF' in box or w.var('mjj').getMax() > 1246:        
        ...
        # paper
#       h_fit_residual_vs_mass.GetXaxis().SetTitle('Dijet mass [TeV]')
        h_fit_residual_vs_mass.GetXaxis().SetTitle('Fourjet Mass [TeV]')
        ...
        xLab.DrawLatex(9000, -4, "9")
        xLab.DrawLatex(10000, -4, "10")        
        xLab.DrawLatex(11000, -4, "11")
        xLab.DrawLatex(12000, -4, "12")
	...
	#print (g_data.GetY()[i-1])
	#if g_data.GetY()[i-1]!=99999 :
