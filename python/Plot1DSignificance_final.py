...
def getThyXsecDict(alphap):    
    ...
    for (mass,thyXsec) in thyXsecDict['Diquark'].items():
...
if __name__ == '__main__':
    ...
    legendLabel = {'RSGravitonGG':'RS graviton (gg#rightarrowG#rightarrowgg)',
                   ...
                   "DMbb1GeV": "DM mediator to bb",
                    "String": "String",
                    "Diquark":"#splitline{Diquark:S_{uu}#rightarrow#chi#chi#rightarrow(qg)(qg)}{m_{#chi}/M_{S} = %s, y_{uu}=0.4, y_{#chi}=0.6}"%(alphatrue),
                   'gg':'gluon-gluon',
                   'qq':'Suu->#chi #chi',
                   'qg':'quark-gluon',
    ...
    if options.doSignificance:
        h_limit.SetTitle(" ;Four-jet resonance mass [GeV];Local significance  [std. deviation]")
    else:
...
#                  expectedLimit_minus2sigma_new[i] = expectedLimit_minus2sigma[(Box,model)][i]
    ...
    h_limit.Draw("a3")
    if options.doSignificance:
        h_limit.GetYaxis().SetNdivisions(8,5,True)
    ...
    l.DrawLatex(0.64,0.92,"%.0f fb^{-1} (13 TeV)"%(options.lumi))
    ...
    if options.doSignificance:
        #lsig.DrawLatex(0.20,0.77,"m_{#chi}/M_{S} = %.2f, y_{uu}=0.4, y_{#chi}=0.6"%(alphatrue))
            lsig.DrawLatex(0.2,0.755,"Y#rightarrow X X #rightarrow (jj) (jj)")
            lsig.SetTextSize(0.03)
            lsig.DrawLatex(0.2,0.68,"M(X)/M(Y) = %.2f"%(alphatrue))
            lsig.DrawLatex(0.2,0.64,"#Gamma/M(Y) = %.1f %%"%(width*100.))
    ...
    if options.doSignificance:
        ...
        if options.bayes:
            ...
            if options.noSys:
                ...
                c.SaveAs(options.outDir+"/limits_freq_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_HN_above_4500_smoothed.pdf")
                c.SaveAs(options.outDir+"/limits_freq_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_HN_above_4500_smoothed.png")
                c.SaveAs(options.outDir+"/limits_freq_"+options.model+"_"+options.box.lower()+"_alpha0p"+str(int(alphap))+"_HN_above_4500_smoothed.C")
