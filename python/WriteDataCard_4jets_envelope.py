...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=True,x=None,emptyHist1D=None):
                    ...
                    arglist.append(w.function(myvar))  
                        ...
                        for iBinX in range(0,maxBins):
                            if w.function('eff_bin%02d'%(iBinX))!=None:
                                ...
                                parlist.add(w.var('eff_bin%02d'%(iBinX)))
                        Utils.importToWS(w,parlist)
                        Utils.importToWS(w,prodlist)
                        prod = rt.RooProdPdf('g_eff','g_eff',prodlist)
                        Utils.importToWS(w,prod)                        
    ...
    w.Print('v')
    if multi:
        paramNames.append('pdf_index')#edw
        bkgs = ['multi']
...
def writeDataCard(box,model,txtfileName,bkgs,paramNames,w,penalty,fixed,shapes=[],multi=False):
        ...
        signals = len(model.split('z'))
        if signals>1:
                rates = [w.data("%s_%s"%(box,sig)).sumEntries() for sig in model.split('z')]
                processes = ["%s_%s"%(box,sig) for sig in model.split('z')]
                if '2015' in box:
                        lumiErrs = [1.027 for sig in model.split('z')]
                elif '2016' in box:
                        lumiErrs = [1.062 for sig in model.split('z')] 
                elif 'Run2' in box:
                        lumiErrs = [1.016 for sig in model.split('z')]  
                elif 'Run3' in box:
                        lumiErrs = [1.016 for sig in model.split('z')]                
        else:
...
def applyTurnonFunc(hist,effFr,w):
    ...
    iterator = RootIterator(effFr.floatParsFinal())
    for p in iterator:
...
#            if f.lower().find('FourjetShape')!=-1:
    ...
    #for i in range(1,myRealTH1.GetNbinsX()+1):
    #    print (myRealTH1.GetBinContent(i), '    ', myRealTH1.GetBinError(i))
    ...
    if options.asimov:
        ...
        #Utils.importToWS(w,triggerData)
        ...
        if isinstance(d, rt.TH1):
           ...
           #  if name=='h_%s_%i'%(model,massPoint):
             if name=='h_FourjetMass': #1GeV
                ...
                Utils.importToWS(w,sigDataHist)
                Utils.importToWS(w,sigDataHist_mjj)
    ...
    if options.inputFitFile is not None:
        ...
        if options.deco:
            ...
            for bkg in bkgs:
                ...
                Utils.importToWS(w,pdf_deco,rt.RooFit.RecycleConflictNodes())
        ...
        if shapeFiles[shape+'Up'] is not None:
            ...
            if options.trigger:
                #  hUp = applyTurnonFunc(fUp.Get('h_%s_%i'%(model,massPoint)),effFrIn,w)
                  hUp = applyTurnonFunc(fUp.Get('h_FourjetMass'),effFrIn,w) #1GeV
            else:
               #  hUp = fUp.Get('h_%s_%i'%(model,massPoint))
                  hUp = fUp.Get('h_FourjetMass') #1GeV
            ...
            Utils.importToWS(w,hUp_DataHist)
        ...
        if shapeFiles[shape+'Down'] is not None: 
            ...
            if options.trigger:
               ...
               #  hDown = fDown.Get('h_%s_%i'%(model,massPoint))
                 hDown = fDown.Get('h_FourjetMass') #1GeV
            ...
            Utils.importToWS(w,hDown_DataHist)
        else:
    ...
    if options.mcFile is not None:
        ...
        Utils.importToWS(w,mcDataHist)
        Utils.importToWS(w,mcDataHist_mjj)
